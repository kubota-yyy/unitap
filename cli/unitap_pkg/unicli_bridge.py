...
def do_unicli(args, _port=None):
    ...
    forwarded = list(args.unicli_args)
    if args.operation in ("exec", "eval") and not forwarded:
    ...
    command = [binary, args.operation, *forwarded]
    if "--json" not in forwarded:
        command.append("--json")
    if args.operation in ("exec", "eval"):
        command.extend(["--timeout", str(args.timeout_ms)])
        if "--no-focus" not in forwarded:
            command.append("--no-focus")
    env = dict(os.environ, UNICLI_PROJECT=str(args.project))
    try:
        completed = subprocess.run(command, cwd=args.project, env=env, capture_output=True,
                                   text=True, timeout=args.timeout_ms / 1000 + 5, check=False)
    ...
    print_unitap_response(args, {"ok": True, "result": {
        "backend": "unicli", "operation": args.operation, "exitCode": completed.returncode,
        "response": response}})
